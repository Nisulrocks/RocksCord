...
export function VerifyEmailNotice({ email, onBack, fromLogin = false }: VerifyEmailNoticeProps) {
  // Registration has just sent one, so the button starts on cooldown in that case.
  const [cooldown, setCooldown] = useState(fromLogin ? 0 : COOLDOWN_SECONDS);
  ...
  return (
    <div className="rounded-panel border border-line bg-surface-2 p-6 shadow-pop">
      <div className="flex flex-col items-center text-center">
        <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-accent/15 text-accent-soft">
          <MailCheck size={24} aria-hidden />
        </div>

        <h2 className="text-lg font-semibold text-ink">
          {fromLogin ? 'Confirm your email to continue' : 'Check your inbox'}
        </h2>
        ...
        <p className="mt-2 text-sm leading-relaxed text-ink-dim">
          ...
          {fromLogin ? ' — open it, then sign in.' : ' to finish signing in.'}
        </p>
